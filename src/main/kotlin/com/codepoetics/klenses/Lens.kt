...
interface Lens<T, V> {
    ...
    operator fun <V2> rem(iso: Iso<V, V2>): Lens<T, V2> = of(
            { t -> iso.arrow(get(t)) },
    ...
    operator fun <V1> plus(next: Lens<V, V1>): Lens<T, V1> = of(
            { t -> next.get(get(t)) },
...
class PropertyMapper<T : Any>(
        ...
        private val members: Array<KProperty1<out Any?, Any?>>,
    ...
    companion object {
        ...
        private fun <T : Any >forKClassUncached(kclass: KClass<T>): PropertyMapper<T> {
            val propertiesByName = kclass.members.filterIsInstance<KProperty1<*, *>>()
                .map { it.name to it }
                .toMap()
            val propertyMappableConstructors = kclass.constructors.filter {
                propertiesByName.keys.containsAll(it.parameters.map { it.name!! }) }
            val constructor = propertyMappableConstructors.minBy { 0 - it.parameters.size }!!
